 <?php
include 'connect.php';
class emp_info extends connect
{
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="emp_info.html" class="nav-link active"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="8" r="3"/><path d="M5 20c0-4 3-6 7-6s7 2 7 6"/></svg></span><span class="link-text">Employee</span></a>
...
themeToggle.addEventListener("click", () => { applyTheme(root.getAttribute("data-theme") === "dark" ? "light" : "dark"); });
	...
	public function input()
	{
		...
		{
			$f = 0;
			$r = mysqli_query($this->db_handle, "SELECT emp_id FROM emp_info");

			while($db_field = mysqli_fetch_assoc($r))
			{
				if($db_field['emp_id'] == $_POST['t1'])
				{
					$f = 1;
					break;
				}
			...
			{
				$sql="insert into emp_info values('$_POST[t1]' , '$_POST[t2]' , '$_POST[t3]' , '$_POST[t4]' , '$_POST[t5]' , '$_POST[t6]', '$_POST[t7]', '$_POST[t8]', '$_POST[t9]', '$_POST[t10]')";
	...
	public function delete()
	{
		if($this->db_handle)
		{
			$sql="delete from emp_info where Emp_ID='$_POST[t1]' ";
			mysqli_query($this->db_handle , $sql);
	...
	public function update()
    {
		...
		{
            $sql = "UPDATE emp_info 
                    SET emp_name='$_POST[t2]',
                        address='$_POST[t3]',
                        status='$_POST[t4]',
                        nationality='$_POST[t5]',
	...
	public function allsearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from emp_info");
				$this->render_header("All Employees");
				echo"<thead><tr>";
				...
				while($db_field=mysqli_fetch_assoc($r))
					{
						...
						echo "<td>" . $db_field['emp_name'] . "</td>";
						echo "<td>" . $db_field['address'] . "</td>";
						echo "<td>" . $db_field['status'] . "</td>";
						echo "<td>" . $db_field['nationality'] . "</td>";
	...
	public function psearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from emp_info where emp_id = '$_POST[t1]'");
				$this->render_header("Employee Search Result");
				echo"<thead><tr>";
				echo"<th> EMPLOYEE ID </th> 
				<th> EMPLOYEE NAME </th>
				<th> ADDRESS </th>
	...
	public function spsearch()
    {
        if ($this->db_handle)
        {
            ...
            if($field=="all")
                $str="SELECT * FROM emp_info";
            else
                $str="SELECT * FROM emp_info WHERE $field='$_POST[t1]'";


            $sql = mysqli_query($this->db_handle, $str);
            $this->render_header("Employee SP Search Result");
            echo "<thead><tr>";
            echo "<th> EMPLOYEE ID </th>";
