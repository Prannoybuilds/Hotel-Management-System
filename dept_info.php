 <?php
include 'connect.php';
class dept_info extends connect
{
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="dept_info.html" class="nav-link active"><span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="5" y="3" width="14" height="18"/><path d="M9 7h2"/><path d="M13 7h2"/><path d="M9 11h2"/><path d="M13 11h2"/><path d="M9 15h6"/></svg></span><span class="link-text">Department</span></a>
...
themeToggle.addEventListener("click", () => { applyTheme(root.getAttribute("data-theme") === "dark" ? "light" : "dark"); });
	...
	public function input()
	{
		...
		{
			$f = 0;
			$r = mysqli_query($this->db_handle, "SELECT dept_id FROM dept_info");

			while($db_field = mysqli_fetch_assoc($r))
			{
				if($db_field['dept_id'] == $_POST['t1'])
				{
					$f = 1;
					break;
				}
			...
			{
				$sql="insert into dept_info values('$_POST[t1]' , '$_POST[t2]' , '$_POST[t3]' , '$_POST[t4]' , '$_POST[t5]' , '$_POST[t6]')";
	...
	public function delete()
	{
		if($this->db_handle)
		{
			$sql="delete from dept_info where Dept_id='$_POST[t1]' ";
			mysqli_query($this->db_handle , $sql);
	...
	public function update()
    {
        if ($this->db_handle) 
		{
            $sql = "UPDATE dept_info
                    SET dept_name='$_POST[t2]',
                        dept_desc='$_POST[t3]',
                        status='$_POST[t4]',
                        created_date='$_POST[t5]',
                        created_by='$_POST[t6]'
                    WHERE dept_id='$_POST[t1]'";
            mysqli_query($this->db_handle, $sql);
	...
	public function allsearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from dept_info");
				$this->render_header("All Departments");
				echo"<thead><tr>";
				...
				while($db_field=mysqli_fetch_assoc($r))
					{
						echo "<tr>";
						echo "<td>" . $db_field['dept_id'] . "</td>";
						echo "<td>" . $db_field['dept_name'] . "</td>";
						echo "<td>" . $db_field['dept_desc'] . "</td>";
	...
	public function psearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from dept_info where dept_id = '$_POST[t1]'");
				$this->render_header("Department Search Result");
				echo"<thead><tr>";
				echo"<th> DEPARTMENT ID </th> 
				<th> DEPARTMENT NAME </th>
				<th> DEPARTMENT DESC </th>
	...
	public function spsearch()
    {
        if ($this->db_handle)
        {
            ...
            if($field=="all")
                $str="SELECT * FROM dept_info";
            else
                $str="SELECT * FROM dept_info WHERE $field='$_POST[t1]'";


            $sql = mysqli_query($this->db_handle, $str);
            $this->render_header("Department SP Search Result");
            echo "<thead><tr>";
            echo "<th> DEPARTMENT ID </th>";
            echo "<th> DEPARTMENT NAME </th>";
            echo "<th> DEPARTMENT DESC </th>";
            echo "<th> STATUS </th>";
