...
class invoice extends connect
{
	...
	private function renderHeader($title)
	{
		...
		echo "<a href='invoice.html' class='nav-link active'><span class='nav-icon'><svg viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.8'><path d='M6 3h9l3 3v15H6z'/><path d='M9 9h6'/><path d='M9 13h6'/><path d='M9 17h4'/></svg></span><span class='link-text'>Invoice</span></a>";
		...
		echo "<a href='spsearch.html' class='search-box search-btn'><svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.8'><circle cx='11' cy='11' r='7'/><path d='M21 21l-4.3-4.3'/></svg><span>Search Invoice</span></a>";
	...
	public function input()
	{
		...
		{
			$f = 0;
			$r = mysqli_query($this->db_handle, "SELECT invoice_id FROM invoice");

			while($db_field = mysqli_fetch_assoc($r))
			{
				if($db_field['invoice_id'] == $_POST['t1'])
				{
					$f = 1;
					break;
				}
			...
			{
				$sql="insert into invoice values('$_POST[t1]' , '$_POST[t2]' , '$_POST[t3]' , '$_POST[t4]' , '$_POST[t5]' , '$_POST[t6]', '$_POST[t7]')";
	...
	public function delete()
	{
		if($this->db_handle)
		{
			$sql="delete from invoice where Invoice_ID='$_POST[t1]' ";
			mysqli_query($this->db_handle , $sql);
	...
	public function update()
    {
		...
		{
            $sql = "UPDATE invoice 
                    SET customer_id='$_POST[t2]',
                        booking_id='$_POST[t3]',
                        discount='$_POST[t4]',
                        gst='$_POST[t5]',
                        created_date='$_POST[t6]',
                        created_by='$_POST[t7]'
                    WHERE invoice_id='$_POST[t1]'";
            mysqli_query($this->db_handle, $sql);
	...
	public function allsearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from invoice");

				$this->renderHeader("All Invoices");
				echo"<tr>";
				...
				while($db_field=mysqli_fetch_assoc($r))
					{
						...
						echo "<td>" . $db_field['discount'] . "</td>";
						echo "<td>" . $db_field['gst'] . "</td>";
	...
	public function psearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from invoice where invoice_id = '$_POST[t1]'");

				$this->renderHeader("Invoice Search Result");
				echo"<tr>";
				echo"<th> INVOICE ID </th> 
				<th> CUSTOMER ID </th>
				<th> BOOKING ID </th>
	...
	public function spsearch()
    {
        if ($this->db_handle)
        {
            ...
            if($field=="all")
                ...
                $str="SELECT * FROM invoice WHERE $field='$_POST[t1]'";
