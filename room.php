...
class room extends connect
{
	...
	/* renderHeader/renderFooter now match the real Customer design system:
	   .sidebar/.sidebar-header/.logo-box/.nav-link/.nav-icon,
	   .topbar/.welcome/.top-right/.theme-toggle,
	   .results-section/.results-page/.results-header/.data-table-wrap/.data-table,
	   .footer with footer-left/footer-center/footer-right/social */
	private function renderHeader($title)
	{
		...
		echo "<a href='room.html' class='nav-link active'><span class='nav-icon'><svg viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.8'><path d='M3 17V8h18v9'/><path d='M3 13h18'/><path d='M7 8V5h4v3'/></svg></span><span class='link-text'>Room</span></a>";
		...
		echo "<a href='payment.html' class='nav-link'><span class='nav-icon'><svg viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.8'><rect x='2.5' y='5' width='19' height='14' rx='2'/><path d='M2.5 10h19'/></svg></span><span class='link-text'>Payment</span></a>";
		...
		echo "<div class='welcome'><span class='eyebrow'>Guest Services</span><h2>{$title}</h2></div>";
		echo "<div class='top-right'>";
		echo "<a href='spsearch.html' class='search-box search-btn'><svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.8'><circle cx='11' cy='11' r='7'/><path d='M21 21l-4.3-4.3'/></svg><span>Search Room</span></a>";
	...
	{
		echo "</table></div>"; // .data-table-wrap
		echo "</div></section>"; // .results-page / .results-section
	...
	public function input()
	{
		...
		{
			$f = 0;
			$r = mysqli_query($this->db_handle, "SELECT room_id FROM room");

			while($db_field = mysqli_fetch_assoc($r))
			{
				if($db_field['room_id'] == $_POST['t1'])
				{
					$f = 1;
					break;
				}
			...
			{
				$sql="insert into room values('$_POST[t1]' , '$_POST[t2]' , '$_POST[t3]' , '$_POST[t4]' , '$_POST[t5]', '$_POST[t6]', '$_POST[t7]')";
	...
	public function delete()
	{
		if($this->db_handle)
		{
			$sql="delete from room where Room_ID='$_POST[t1]' ";
			mysqli_query($this->db_handle , $sql);
	...
	public function update()
    {
		...
		{
            $sql = "UPDATE room 
                    SET hotel_code='$_POST[t2]',
                        room_type_id='$_POST[t3]',
						total_days='$_POST[t4]',
						room_charge='$_POST[t5]',
                        created_date='$_POST[t6]',
                        created_by='$_POST[t7]'
                    WHERE room_id='$_POST[t1]'";
            mysqli_query($this->db_handle, $sql);
	...
	public function allsearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from room");

				$this->renderHeader("All Rooms");
				echo"<tr>";
				echo"<th> ROOM ID </th> 
				<th> HOTEL CODE </th>
				<th> ROOM TYPE ID </th>
				...
				while($db_field=mysqli_fetch_assoc($r))
					{
						...
						echo "<td>" . $db_field['room_type_id'] . "</td>";
						echo "<td>" . $db_field['total_days'] . "</td>";
						echo "<td>" . $db_field['room_charge'] . "</td>";
	...
	public function psearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from room where room_id = '$_POST[t1]'");

				$this->renderHeader("Room Search Result");
				echo"<tr>";
				echo"<th> ROOM ID </th> 
				<th> HOTEL CODE </th>
				<th> ROOM TYPE ID </th>
	...
	public function spsearch()
    {
        if ($this->db_handle)
        {
            ...
            if($field=="all")
                ...
                $str="SELECT * FROM room WHERE $field='$_POST[t1]'";
...
/* ================= YOUR ORIGINAL: routing block, untouched ================= */
$obj=new room();
