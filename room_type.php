 <?php
include 'connect.php';
class room_type extends connect
{
	...
	private function renderHeader($title)
	{
		...
		echo "<a href='room_type.html' class='nav-link active'><span class='nav-icon'><svg viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.8'><rect x='4' y='3' width='16' height='18' rx='2'/><path d='M8 7h2'/><path d='M14 7h2'/><path d='M8 11h2'/><path d='M14 11h2'/><path d='M8 15h8'/></svg></span><span class='link-text'>Room Type</span></a>";
		...
		echo "<a href='spsearch.html' class='search-box search-btn'><svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.8'><circle cx='11' cy='11' r='7'/><path d='M21 21l-4.3-4.3'/></svg><span>Search Room Type</span></a>";
	...
	public function input()
	{
		...
		{
			$f = 0;
			$r = mysqli_query($this->db_handle, "SELECT room_type_id FROM room_type");

			while($db_field = mysqli_fetch_assoc($r))
			{
				if($db_field['room_type_id'] == $_POST['t1'])
				{
					$f = 1;
					break;
				}
			...
			{
				$sql="insert into room_type values('$_POST[t1]' , '$_POST[t2]' , '$_POST[t3]' , '$_POST[t4]' , '$_POST[t5]', '$_POST[t6]')";
	...
	public function delete()
	{
		if($this->db_handle)
		{
			$sql="delete from room_type where Room_type_id='$_POST[t1]' ";
			mysqli_query($this->db_handle , $sql);
	...
	public function update()
    {
        if ($this->db_handle) 
		{
            $sql = "UPDATE room_type 
                    SET no_of_bed='$_POST[t2]',
                        type_name='$_POST[t3]',
						status='$_POST[t4]',
                        created_date='$_POST[t5]',
                        created_by='$_POST[t6]'
                    WHERE room_type_id='$_POST[t1]'";
            mysqli_query($this->db_handle, $sql);
	...
	public function allsearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from room_type");

				$this->renderHeader("All Room Types");
				echo"<tr>";
				...
				while($db_field=mysqli_fetch_assoc($r))
					{
						...
						echo "<td>" . $db_field['no_of_bed'] . "</td>";
						echo "<td>" . $db_field['type_name'] . "</td>";
	...
	public function psearch()
	{
		if($this->db_handle)
			{
				$r=mysqli_query($this->db_handle,"select * from room_type where room_type_id = '$_POST[t1]'");

				$this->renderHeader("Room Type Search Result");
				echo"<tr>";
				echo"<th> ROOM TYPE ID </th> 
				<th> NO OF BED </th>
				<th> TYPE NAME </th>
	...
	public function spsearch()
    {
        if ($this->db_handle)
        {
            ...
            if($field=="all")
                $str="SELECT * FROM room_type";
            else
                $str="SELECT * FROM room_type WHERE $field='$_POST[t1]'";
